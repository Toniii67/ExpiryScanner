...
struct CameraView: View {
    ...
    var body: some View {
        ZStack {
            ...
            OverlayView(
                ...
                showDoneAlert: $viewModel.showDoneAlert,
                viewModel: viewModel,
                onCustomAlertDismiss: handleCustomAlertDismiss
            )
        ...
        .onChange(of: viewModel.isSessionRunning) { newValue in handleSessionRunningChange(newValue) }
        .onChange(of: viewModel.detectedProductName) { _ in checkForDetectionCompletion() }
        .onChange(of: viewModel.detectedExpiryDate) { _ in checkForDetectionCompletion() }
    }
        ...
        var body: some View {
            VStack {
                if showCustomAlert {
                    CustomAlertView(
                        text: NSLocalizedString("Silakan tahan posisi Anda dan putar item secara perlahan ke segala arah selama 10 detik saat proses pemindaian berlangsung", comment: "Custom alert text"),
                        onDismiss: onCustomAlertDismiss
                    )
                    .padding(.top, 20)
                }
            ...
            .background(
                NativeAlertController(
                    ...
                    message: {
                        if viewModel.detectedProductName == nil && viewModel.detectedExpiryDate == nil {
                            return NSLocalizedString("Produk tidak ditemukan", comment: "None detected text")
                        } else if let name = viewModel.detectedProductName, let date = viewModel.detectedExpiryDate {
                            return NSLocalizedString("Hasil deteksi objek: \(name), \(date.formatted(date: .long, time: .omitted))", comment: "Both detected text")
                        } else {
                            return NSLocalizedString("Hasil deteksi objek: \(viewModel.detectedProductName ?? "data tidak ditemukan"), \(viewModel.detectedExpiryDate?.formatted(date: .long, time: .omitted) ?? "data tidak ditemukan")", comment: "Partial detected text")
                        }
                    }(),
                    viewModel: viewModel
                )
    ...
    private struct LoadingView: View {
        ...
        var body: some View {
            if isLoading && ProcessInfo.processInfo.environment["XCODE_RUNNING_FOR_PREVIEWS"] != "1" {
    ...
    private var guidanceAlert: Alert {
        Alert(
            title: Text(NSLocalizedString("Petunjuk Pemindaian", comment: "Guidance alert title")),
            message: Text(currentGuidanceText),
            dismissButton: .default(Text(NSLocalizedString("OK", comment: "Dismiss button"))) {
    ...
    private func handleCustomAlertDismiss() {
        triggerSuccessHaptic()
        showCustomAlert = false
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
            print("CustomAlertView: onDismiss, triggering final alert")
    ...
    private func handleSessionRunningChange(_ newValue: Bool) {
        if newValue {
            isLoading = false
        }
    }

    private func checkForDetectionCompletion() {
        guard !showCustomAlert, !viewModel.showAlert else { return }
        print("checkForDetectionCompletion: product=\(String(describing: viewModel.detectedProductName)), date=\(String(describing: viewModel.detectedExpiryDate))")
        if viewModel.detectedProductName != nil || viewModel.detectedExpiryDate != nil {
            viewModel.showAlert = true
        }
    }
        ...
        var body: some View {
            if isVisible {
                Text(text)
                    ...
                    .frame(maxWidth: 280)
                    .multilineTextAlignment(.center)
                    .accessibilityLabel(NSLocalizedString("Pemindaian dalam proses", comment: "Custom alert accessibility label"))
                    .accessibilityHint(NSLocalizedString("Peringatan ini akan hilang dalam 10 detik.", comment: "Custom alert accessibility hint"))
                    .onAppear {
                        print("CustomAlertView: Appeared")
                        DispatchQueue.main.asyncAfter(deadline: .now() + 10) {
                            if isVisible {
                                onDismiss()
                                isVisible = false
